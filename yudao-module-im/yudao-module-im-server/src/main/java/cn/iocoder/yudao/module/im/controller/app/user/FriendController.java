...
import cn.iocoder.yudao.module.im.controller.admin.userfriends.vo.UserFriendsApplyRespVO;
import cn.iocoder.yudao.module.im.controller.admin.userfriends.vo.UserFriendsPageReqVO;
import cn.iocoder.yudao.module.im.controller.admin.userfriends.vo.UserFriendsRespVO;
...
import cn.iocoder.yudao.module.im.controller.app.chat.vo.ChatRespVO;
import cn.iocoder.yudao.module.im.controller.app.user.vo.FriendApplyHandleReqVO;
import cn.iocoder.yudao.module.im.controller.app.user.vo.FriendApplyReqVO;
...
//    }
    ...
    @Parameter(name = "targetUid", description = "编号", required = true, example = "1024")
    public CommonResult<UserInfoRespVO> applyFriend(@RequestBody FriendApplyReqVO applyReqVO) {
        UserInfoDO userInfo = userInfoService.getUserInfo(applyReqVO.targetUid());
        if(userInfo == null){
            return error(1,"用户不存在");
        }
        userFriendsService.createUserFriendsApply(SecurityFrameworkUtils.getLoginUserId(),applyReqVO.targetUid(),applyReqVO.msg());
        return success(BeanUtils.toBean(userInfo, UserInfoRespVO.class));
    }

    @PostMapping("/apply/handle")
    @Operation(summary = "申请添加好友结果处理")
    public CommonResult<Integer> applyFriendRst(@RequestBody FriendApplyHandleReqVO applyReqVO) {
        UserFriendsApplyDO applyInfo = userFriendsService.getApplyInfo(applyReqVO.applyId());
        if(applyInfo == null){
            return error(1,"申请记录不存在");
        }
        applyInfo.setUserId(SecurityFrameworkUtils.getLoginUserId());
        applyInfo.setStatus(applyReqVO.status());
        int n = userFriendsService.handleUserFriendsApply(applyInfo,applyReqVO.status());
        return success(n);
    }

    @GetMapping("/apply/page")
    @Operation(summary = "申请加好友的列表-新朋友")
    public CommonResult<PageResult<UserFriendsApplyRespVO>> getChatFriendApplyPage(@Valid UserFriendsPageReqVO pageReqVO) {
        pageReqVO.setUserId(SecurityFrameworkUtils.getLoginUserId());
        PageResult<UserFriendsApplyRespVO> pageResult = userFriendsService.getUserFriendsApplyPage(pageReqVO);
        if(pageResult.getList().size() == pageReqVO.getPageSize()){
//            chatRespVO.setCursor(pageReqVO.getCursor()+1);
            pageResult.setIsLast(false);
        }else{
//            chatRespVO.setCursor(pageReqVO.getCursor());
            pageResult.setIsLast(true);
        }
        pageResult.setCurrent(pageReqVO.getPageNo());
        pageResult.setPageSize(pageReqVO.getPageSize());
        return success(pageResult);
    }

    @PostMapping("/search/page")
    @Operation(summary = "搜索好友")
    public CommonResult<PageResult<UserInfoRespVO>> getSearchPage(@RequestBody UserSearchPageReqVO pageReqVO) {

        PageResult<UserInfoRespVO> pageResult = userFriendsService.searchUserFriendsPage(pageReqVO);
        if(pageResult.getList().size() == pageReqVO.getPageSize()){
//            chatRespVO.setCursor(pageReqVO.getCursor()+1);
            pageResult.setIsLast(false);
        }else{
//            chatRespVO.setCursor(pageReqVO.getCursor());
    ...
    @PostMapping("/check")
    @Operation(summary = "搜索好友")
    public CommonResult<List<UserInfoRespVO>> checkFriend(@RequestBody JSONObject body) {
        List<Long> uidList = body.<List<Long>>getObject("uidList", List.class);
        List<UserInfoDO> userInfoList = userInfoService.getUserInfo(uidList);
        return success(BeanUtils.toBean(userInfoList, UserInfoRespVO.class));
    }
