 ...
 */
@Mapper
public interface UserFriendsApplyMapper extends BaseMapperX<UserFriendsApplyDO> {

    default PageResult<UserFriendsApplyDO> selectPage(UserFriendsPageReqVO reqVO) {
        return selectPage(reqVO, new LambdaQueryWrapperX<UserFriendsApplyDO>()
                .eqIfPresent(UserFriendsApplyDO::getUserId, reqVO.getUserId())
                .eqIfPresent(UserFriendsApplyDO::getFriendId, reqVO.getFriendId())
                .eqIfPresent(UserFriendsApplyDO::getStatus, reqVO.getStatus())
                .betweenIfPresent(UserFriendsApplyDO::getCreateTime, reqVO.getCreateTime())
                .orderByDesc(UserFriendsApplyDO::getId));
    }
...
//
//    @Select("SELECT u.USERNAME,u.NAME,u.NICK_NAME,u.FIRST_CHAR,u.AVATAR,u.AGE,u.BIRTHDAY,u.AREA,f.* FROM IM_USER_INFO u inner join IM_USER_FRIENDS_APPLY f on u.ID = f.USER_ID  where f.DELETED  = 0 and f.USER_ID = #{userId}")
